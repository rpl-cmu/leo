...
#ifndef PLANAR_SDF_H_
#define PLANAR_SDF_H_

#include <gtsam/base/Matrix.h>
#include <gtsam/geometry/Point2.h>
#include <leo/3rdparty/gpmp2/SDFexception.h>
...
class PlanarSDF {
 ...
 public:
  ...
  PlanarSDF(const gtsam::Point2& origin, double cell_size, const gtsam::Matrix& data) : origin_(origin), field_rows_(data.rows()), field_cols_(data.cols()), cell_size_(cell_size), data_(data) {}
  ...
  inline double signed_distance(const float_index& idx) const {
    ...
    return (hr - idx.get<0>()) * (hc - idx.get<1>()) * signed_distance(lri, lci) +
