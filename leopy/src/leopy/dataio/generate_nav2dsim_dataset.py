...
def create_measurements(cfg, poses, covariances):
    num_steps = poses.shape[0]

    odom_noise = gtsam.noiseModel_Diagonal.Sigmas(covariances.odom)
    gps_noise = gtsam.noiseModel_Diagonal.Sigmas(covariances.gps)

    sampler_odom_noise = gtsam.Sampler(odom_noise, 0)
    sampler_gps_noise = gtsam.Sampler(gps_noise, 0)
...
def log_data(params, poses, measurements, save_file=False):
    ...
    for tstep in range(0, num_steps):
        ...
        if (tstep > 0):
            ...
            factor_meas.append(measurements.odom[tstep-1].tolist())
        ...
        factor_meas.append(measurements.gps[tstep].tolist())
...
def get_covariances(params):
    ...
    if (params.measurements.noise_models == "random"):
        covariances.odom = random_cov_sigmas(min_val=1e-2, max_val=1e-1, dim=3)
        covariances.gps = random_cov_sigmas(min_val=1e-1, max_val=1, dim=3)

        return covariances

    covariances.odom = np.array(params.measurements.noise_models.odom)
    covariances.gps = np.array(params.measurements.noise_models.gps)
