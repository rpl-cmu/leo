...
def add_qs_motion_factor(graph, keys, factor_cov, params=None):
    ...
    factor = leocpp.QSVelPushMotionRealObjEEFactor(
...
def add_sdf_intersection_factor(graph, keys, factor_cov, object_sdf):
    ...
    factor_noise_model = get_noise_model(factor_cov)
    factor = leocpp.IntersectionPlanarSDFObjEEFactor(
...
def add_tactile_rel_meas_factor(graph, keys, factor_cov, tf_pred, params=None):
    ...
    factor_noise_model = get_noise_model(factor_cov)
    factor = leocpp.TactileRelativeTfPredictionFactor(
...
## fixed tactile model weights
def tactile_model_output_fixed(tactile_model, img_feats, params=None):
...
## learnable tactile model weights
def tactile_model_output(cost, img_feats, params=None):

    tf_pred = cost.theta.tactile_model_output(img_feats, params)
    tf_pred = (tf_pred.view(-1)).detach().cpu().numpy()
...
def log_step_df_push2d(tstep, dataframe, data, optimizer):
    ...
    # add to dataframe
    data_dict = {'opt/graph/obj_poses2d': [obj_pose_vec_graph.tolist()],
