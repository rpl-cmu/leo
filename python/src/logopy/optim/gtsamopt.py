...
from logopy.optim.nav2d_factors import *
from logopy.optim.push2d_factors import *
...
def save_logger_to_file(params, logger):
    logger_dir = "{0}/local/logger/{1}/logo_itr_{2:04d}".format(
        params.BASE_PATH, params.dataio.dataset_name, params.logo.itr)

    filename = "{0}/{1}.obj".format(logger_dir, params.dataio.data_idx)
    dir_utils.save_pkl_obj(filename, logger)

    print("[gtsamopt::save_logger_to_file] Save logger to {0}".format(filename))
...
def run_optimizer(cost, params=None):
    ...
    for tstep in range(1, nsteps):
        ...
        factor_keysyms = [cost.data['factor_keysyms'][idx]
                          for idx in factor_idxs]
        factor_keyids = [cost.data['factor_keyids'][idx]
                         for idx in factor_idxs]
            ...
            if (factor_name == 'gps'):
                ...
                if (params.tactile_model.oracle == True):
                    ...
                    tf_pred_net = tactile_model_output(
                        tactile_model, img_feats=factor_meas[idx], params=params.tactile_model)
    ...
    mean, cov = get_optimizer_soln(tstep, graphopt, dataset_type, params.logo.sampler)

    if (params.optim.save_logger):
        save_logger_to_file(params, graphopt.logger)
