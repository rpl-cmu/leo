...
def vis_step_nav2d(tstep, logger, params=None):
    ...
    plt.plot(poses_graph[0:tstep, 0], poses_graph[0:tstep, 1], linewidth=3,
                color=params.plot.colors.opt, label=params.plot.labels.opt)
    ...
    if params.optim.save_fig:
        filename = "{0}/{1}/{2}/{3}/{4}/{5:04d}.png".format(
            params.BASE_PATH, params.plot.dstdir, params.dataio.dataset_name, params.dataio.prefix, params.dataio.data_idx, params.logo.itr)
...
def vis_step_push2d(tstep, logger, params=None):
    ...
    if params.optim.save_fig:
        filename = "{0}/{1}/{2}/{3}/{4}/{5:04d}.png".format(
            params.BASE_PATH, params.plot.dstdir, params.dataio.dataset_name, params.dataio.prefix, params.dataio.data_idx, params.logo.itr)
        # print("[vis_utils::vis_step_push2d] Save figure to {0}".format(filename))
        dir_utils.save_plt_fig(plt, filename, mkdir=True)

def vis_expert_push2d(poses_obj_gt, poses_ee_gt, params=None):
    ...
    if True:
        filename = "{0}/{1}/{2}/{3}/{4}/expert.png".format(
            params.BASE_PATH, params.plot.dstdir, params.dataio.dataset_name, params.dataio.prefix, params.dataio.data_idx)
        print("[vis_utils::vis_expert_push2d] Save figure to {0}".format(filename))
        dir_utils.save_plt_fig(plt, filename, mkdir=True)
